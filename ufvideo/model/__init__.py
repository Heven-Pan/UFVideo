...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False,lora=False, args=None, **kwargs):
    if 'token' in kwargs:
        token = kwargs['token']
    else:
        token = None
    ...
    config.sam_pretrained='sam2-hiera-large/sam2_hiera_large.pt'
    config.sam_out_dim=256
    ...
    if lora == True:
        ...
        tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False, token=token, local_files_only=True)
        model = VideoReferQwen2ForCausalLM.from_pretrained(model_base, local_files_only=True, low_cpu_mem_usage=True, config=base_config, **kwargs)

        from peft import PeftModel
        # model.init_special_embeddings()
        # model.initialize_embedings(2, model.config.vocab_size+2)
        ...
        def load_lora(model, lora_path):
                ...
                if os.path.exists(non_lora_trainables_path):
                    ...
                    print(f"Load non_lora_trainables unexpected_keys: {info[1]}")
                print('Loading pretrained LoRA weights...')
                model = PeftModel.from_pretrained(model, lora_path)
                model = model.merge_and_unload()
                print('Have merged pretrained LoRA weights...')
                return model
        ...
        if model_type in ['videorefer', 'videorefer_qwen2']:
            ...
            model = VideoReferQwen2ForCausalLM.from_pretrained(model_path, local_files_only=True, low_cpu_mem_usage=True, config=config, **kwargs)
        else:
            raise NotImplementedError
    else:
    ...
    # model.get_model().initialize_sam_modules(args)
