...
class VideoReferMetaModel:

    def __init__(self, config):
        super(VideoReferMetaModel, self).__init__(config)
    ...
    def initialize_sam_modules(self, config):
        # SAM
        self.mask_encoder = SAM2(ckpt_path=config.sam_pretrained) #config.sam_pretrained, None
        ...
        self.mask_encoder.sam2_model.requires_grad_(False)
        if config.train_mask_decoder:
            self.mask_encoder.sam2_model.sam_mask_decoder.train()
            self.mask_encoder.sam2_model.sam_mask_decoder.requires_grad_(True)
        else:
            self.mask_encoder.sam2_model.sam_mask_decoder.eval()

        # Projection layer
        in_dim = config.hidden_size
        out_dim = config.sam_out_dim
        text_fc = [
...
class VideoReferMetaForCausalLM(ABC):
    ...
    def encode_images_or_videos(self, images):
        ...
        # print('data type: ', data_batch.dtype)
        # print('model weight type: ', self.model.region_encoder.feat_linear[0].weight.dtype)
        # print('data_batch shape: ', data_batch.shape)
        frames = einops.rearrange(data_batch, 'b t c h w -> (b t) c h w')
        frames_features = self.get_model().get_vision_tower()(frames)
        # print('frames_features shape: ', frames_features.shape)
    ...
    def prepare_inputs_labels_for_multimodal(
        self, input_ids, attention_mask, past_key_values, labels, images, masks, frame, ann_indices, frame_nums, video_file=''
    ):
        ...
        if frame is not None:
            frame_cns = torch.cat(frame, dim=0)
            # print("frame_cns: ", frame_cns.shape)
            # print('file:', video_file)
            first_frame_features = self.get_model().get_vision_tower()(frame_cns)
            # print('first_frame_features: ', first_frame_features.shape, 'masks: ', masks[0].shape, "mm_features: ", mm_features.shape, "ann_indices: ", ann_indices, "frame_nums: ", frame_nums)
            # sys.exit(0)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            last_embeds = cur_new_input_embeds[-1].shape[0]
            cur_new_input_embeds = [x.to(device=self.device) for x in cur_new_input_embeds]
            # NOTE: one cur_new_input_embeds per each  
            cur_new_input_embeds = torch.cat(cur_new_input_embeds, dim=0)
            mark_mm_token_indices.append([cur_new_input_embeds.shape[0] - last_embeds, last_embeds])
        ...
        return None, attention_mask, past_key_values, new_input_embeds, new_labels, mark_mm_token_indices


    def initialize_MM_tokenizer(self, tokenizer):
        # add region token
        _ = tokenizer.add_tokens('<region>', special_tokens=True)
        self.temporal_tokens = [TEMPORAL_TOKEN_FORMAT.format(i) for i in range(100)]
        _ = tokenizer.add_tokens(self.temporal_tokens, special_tokens=True)
        _ = tokenizer.add_tokens('[SEG]', special_tokens=True)
