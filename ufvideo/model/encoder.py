...
class SiglipVisionTower(nn.Module):
    ...
    def __init__(self, vision_tower, args, delay_load=False):
        ...
        self.path = 'siglip-so400m-patch14-384'

        if not delay_load:
            self.load_model()
        else:
            # self.cfg_only = SiglipVisionConfig.from_pretrained(self.vision_tower_name, local_files_only=True)
            self.cfg_only = SiglipVisionConfig.from_pretrained(self.path, local_files_only=True)
    ...
    def load_model(self):
        ...
        self.image_processor = SiglipImageProcessor.from_pretrained(self.path, local_files_only=True)
        self.vision_tower = SiglipVisionModel.from_pretrained(self.path, local_files_only=True)
        self.vision_tower.requires_grad_(False)
