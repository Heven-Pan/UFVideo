...
class SAM2(nn.Module):
    ...
    def inject_language_embd_train(self, sam_states, language_embd, nf_nobj=None):
        high_res_features = [
    ...
    def language_embd_inference(self, inference_state, language_embd): # 5*[(1, 256)]
        num_frame = len(language_embd) # 5
        num_obj = len(language_embd[0]) # 1
        mask_out = []
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            for frame_idx in range(num_frame):
                frame_mask_out = []
                ...
                for obj_idx in range(num_obj):
                    _language_embd = language_embd[frame_idx][obj_idx][None][None] # (1, 1, 256)
            ...
            for out_frame_idx, out_obj_ids, out_mask_logits in self.sam2_model.propagate_in_video(inference_state):
                mask_out.append(out_mask_logits) # frame_idx, 100, (1, 1, 1024, 1024)
            mask_out = torch.cat(mask_out, dim=0)
        return mask_out
    ...
    def get_sam2_embeddings_inference(self, images):
        return self.sam2_model.init_state(images)

    def get_sam2_embeddings_train(self, images, expand_size=1, obj_num_list=[], BS=2, T=4):
        ...
        for i, feat in enumerate(feats["backbone_fpn"]):
            ...
            for bs in range(BS):
                if obj_num_list[bs] == 0:
                    feat_bs_all.append(feat[(bs * T):((bs+1) * T), None].expand(-1, 1, -1, -1, -1).flatten(0, 1))
                else:
                    feat_bs_all.append(feat[(bs * T):((bs+1) * T), None].expand(-1, obj_num_list[bs], -1, -1, -1).flatten(0, 1))
            feats["backbone_fpn"][i] = torch.cat(feat_bs_all)
        ...
        for i, pos in enumerate(feats["vision_pos_enc"]):
            ...
            for bs in range(BS):
                if obj_num_list[bs] == 0:
                    vision_pos_all.append(pos[(bs * T):((bs+1) * T), None].expand(-1, 1, -1, -1, -1).flatten(0, 1))
                else:
                    vision_pos_all.append(pos[(bs * T):((bs+1) * T), None].expand(-1, obj_num_list[bs], -1, -1, -1).flatten(0, 1))
            # pos = pos[:, None].expand(-1, expand_size, -1, -1, -1).flatten(0, 1)
            feats["vision_pos_enc"][i] = torch.cat(vision_pos_all)
...
class SAM2Base(SAM2Base_):
    ...
    def _forward_sam_heads(
        ...
        if language_embd is not None:
            # B N C
            # sparse_embeddings:  torch.Size([12, 2, 256]), language_embd:  torch.Size([16, 1, 256])
            try:
                ...
                print("language_embd: ", language_embd.shape)
                print('backbone_feature:', backbone_features.shape)
...
class SAM2VideoPredictor(SAM2Base):
    ...
    def add_language_embd(
        ...
        obj_idx = _obj_id_to_idx(inference_state, obj_id) # 0
    ...
    def propagate_in_video(
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video", leave=False):
