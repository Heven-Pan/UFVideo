...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ['mm_projector', 'vision_tower', 'vision_resampler', 'region_encoder']
...
def safe_save_model_for_hf_trainer(trainer: Trainer,
    ...
    if getattr(trainer.args, "tune_region_encoder", False):
        # Only save Adapter
        keys_to_match = ['region_encoder']
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            if current_folder.startswith('checkpoint-'):
                mm_projector_folder = os.path.join(parent_folder, "region_encoder")
                os.makedirs(mm_projector_folder, exist_ok=True)
                torch.save(weight_to_save, os.path.join(mm_projector_folder, f'{current_folder}.bin'))
            else:
                torch.save(weight_to_save, os.path.join(output_dir, f'region_encoder.bin'))
        return
...
class VideoReferTrainer(Trainer):
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            # print('lr:', self.args.mm_projector_lr)
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
        ...
        elif getattr(self.args, 'tune_region_encoder', False):
            from transformers.trainer_utils import PREFIX_CHECKPOINT_DIR
            checkpoint_folder = f"{PREFIX_CHECKPOINT_DIR}-{self.state.global_step}"
            ...
            if self.args.lora_enable:
                ...
                super(VideoReferTrainer, self)._save_checkpoint(model, trial, metrics)
            else:
                super(VideoReferTrainer, self)._save_checkpoint(model, trial, metrics)
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            super(VideoReferTrainer, self)._save(output_dir, state_dict)
