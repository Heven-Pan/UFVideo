...
def process_sample(data, pred_mask_root):
    ...
    for num_obj, subdir in enumerate(subdirs):
        ...
        pred_idxs = [os.path.basename(x.split('.png')[0]) for x in pred_mask_paths]
        gt_mask_list = []
        for p_idx in pred_idxs:
            gt_mask_list.append(gt_anno[str(p_idx)])
    
        if not pred_mask_paths:
            return (sample_id, [], [], [])

        # if len(pred_mask_paths) < len(gt_mask_list):
