...
def collate_fn(batch):
    ...
    return vin, vid, msk, qs, fr, aid, fn, ans, tps, img_sam, h, w

def build_videorefer_bench_q_eval(args, processor, local_rank):
...
def run_inference(args):
    ...
    val_loader = build_videorefer_bench_q_eval(args, processor, local_rank)
    
    final_data = []
    for i, (video_names, video, masks_, questions, frames, ann_ids, framenums, answers, types, img_sam, h, w) in enumerate(tqdm(val_loader, desc=f"Rank {global_rank}", position=local_rank)):
