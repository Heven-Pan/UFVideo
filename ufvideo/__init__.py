...
def model_init(model_path=None, region=None,lora=False, args=None, **kwargs):
    model_name = get_model_name_from_path(model_path)
    tokenizer, model, processor, context_len = load_pretrained_model(model_path, None, model_name, lora=lora, args=args, **kwargs)
...
def mm_infer(image_or_video, instruct, model, tokenizer, modal='video', masks=None, ann_indices=None, frame_nums=None, frame=None, choice=1, images_sam=None,
            offset=None,
            masks_list=None,
            label_list=None,
            seg=False, **kwargs):
    """inference api of VideoRefer for video understanding.

    Args:
        model: VideoRefer model.
    ...
    if choice == 1:
        ...
        message = [{'role': sentence['from'][0], 'content': sentence['value'][0]} for sentence in instruct]

    if model.config.model_type in ['videorefer', 'videorefer_mistral', 'videorefer_mixtral']:
        system_message = [
            {'role': 'system', 'content': (
    ...
    # do_sample为True引入随机，temp变高会引入随机
    ...
    # 如果return_dict = True, output_ids.logits.shape = 1, 1436, 151748, output_ids.hidden_states[-1].shape = 1 1436, 3584
    if not seg:
        outputs = tokenizer.batch_decode(output_ids['output'], skip_special_tokens=True)[0].strip()
